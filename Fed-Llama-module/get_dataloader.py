...
def get_dataset(args, tokenizer):
    ...
    elif args.do_predict and not args.do_inference:
    ...
    def preprocess_function_eval(examples):
        ...
        for i in range(len(examples[passage_column])):
            ...
            if examples[passage_column][i] : # :and examples[answer_column][i]
                ...
                label_ids = labels_ids + [tokenizer.eos_token_id]
    ...
    if args.do_train:
        ...
        #     input_ids = sample['input_ids']  # 假设 'input_ids' 是你数据集中的一个键
        #     # 删除填充标记
        #     input_ids_filtered = [id for id in input_ids if id != 0]
        #     if len(input_ids_filtered) > max_length:
        #         max_length = len(input_ids_filtered)
        #         max_sample = sample

        # print(f"最大 input_ids 长度: {max_length}")
        # print("对应的样本:", max_sample)        
    ...
    if args.do_predict:
        if args.do_inference:
            ...
            seq_len = 130 #前向传播推理时 输入的最大tokens限制
