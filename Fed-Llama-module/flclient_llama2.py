...
'''=====================================================设置日志====================================================='''
log_path = "/home/zhangzishuai/SplitFederated-LLaMA/log/FedLlama/train_client_1.log"
...
def FLparser(): # use glm arguments with argument.py
    ...
    '''=====================================================model args====================================================='''                    
    # 
    parser.add_argument("--modelA_name_or_path", type=str, default="/home/zhangzishuai/SplitFederated-LLaMA/For_Open_Source/Fed-Llama-module/client",
                        help="The model checkpoint for weights initialization. Don't set if you want to train a model from scratch.")
    parser.add_argument("--modelC_name_or_path", type=str, default="/home/zhangzishuai/SplitFederated-LLaMA/For_Open_Source/Fed-Llama-module/client_part3",
                        help="The model checkpoint for weights initialization. Don't set if you want to train a model from scratch.")
    ...
    parser.add_argument("--output_dir", type=str, help = 'output folder path', default="/home/zhangzishuai/SplitFederated-LLaMA/For_Open_Source/Fed-Llama-module/output/Friends/ReadCompre/version_1")
    ...
    # parser.add_argument("--train_file", type=str,  default='reading-comprehension-trn-part1.json')
...
class Fed_Client(NumPyClient):
    ...
    def fit_partA(self, parameters: NDArrays=None, current_step=0, need_test_data=0):
        ...
        elif self.FL_args.do_eval:  #目前没有开发eval
        ...
        # #查看精度损失
        # stored_path = "/home/zhangzishuai/SplitFederated-LLaMA/pickle-store/od_tensor_client.pickle"
        ...
        # #查看精度损失
        # stored_path = "/home/zhangzishuai/SplitFederated-LLaMA/pickle-store/od_numpy_client.pickle"
        # with open(stored_path, 'wb') as f:
    ...
    def fit_partC(self, parameters: NDArrays=None, config: Dict[str, Scalar]=None , child_conn=None):       
        ...
        # 6/26新增
            ...
            self.data , self.attn_mask = self.concatenate_output(self.data, pred_ids)
    ...
    def concatenate_output(self, input_tensor, output_tensor ):
        pad_token=3
        # 获取输入的形状
        ...
        if pad_length >= 0:
            attn_mask = torch.cat((torch.zeros(pad_length, dtype=torch.int64), torch.ones(new_input.size(1), dtype=torch.int64)),dim=0)
            ...
            padded_input = new_input[:, 1:] #舍弃掉一个旧的，要新的concate的
        return padded_input, attn_mask 
...
def main(prompt: str =None , child_conn=None) -> None:
    ...
    FL_args.max_predict_samples = 200 # 样本最大数
    FL_args.prompt = prompt
    #FL_args.lora_modelA_path = '/home/zhangzishuai/SplitFederated-LLaMA/For_Open_Source/Fed-Llama-module/output/Friends/ReadCompre/model-A/checkpoint-1000'
    #FL_args.lora_modelC_path = '/home/zhangzishuai/SplitFederated-LLaMA/For_Open_Source/Fed-Llama-module/output/Friends/ReadCompre/model-C/checkpoint-1000'
    ...
    if FL_args.do_train:
        ...
        if FL_args.modelA_name_or_path:
            ...
            # clientA_state_dict = torch.load(f'{FL_args.modelA_name_or_path}/pytorch_model_A.bin', map_location='cpu')
        ...
        if FL_args.modelC_name_or_path:
            ...
            # clientC_state_dict = torch.load(f'{FL_args.modelC_name_or_path}/pytorch_model_C.bin', map_location='cpu')
        ...
        if FL_args.modelA_name_or_path:
            ...
            #model_A = LlamaForCausalLM(config=configA).to('cuda:0')#.to(device)
            clientA_state_dict = torch.load(f'{FL_args.modelA_name_or_path}/pytorch_model_A.bin', map_location='cuda:1')
            # clientA_state_dict = torch.load(f'{FL_args.modelA_name_or_path}/pytorch_model_A.bin', map_location='cpu')
            model_A.load_state_dict(clientA_state_dict)
            del clientA_state_dict
        ...
        if FL_args.modelC_name_or_path:
            ...
            #model_C = LlamaForCausalLMC(config=configC).to('cuda:0')#.to(device) 
            clientC_state_dict = torch.load(f'{FL_args.modelC_name_or_path}/pytorch_model_C.bin', map_location='cuda:1')
            # clientC_state_dict = torch.load(f'{FL_args.modelC_name_or_path}/pytorch_model_C.bin', map_location='cpu')
            model_C.load_state_dict(clientC_state_dict)
            del clientC_state_dict
        ...
        if FL_args.AggregatedClients:
            ...
            if FL_args.AggregatedAvg:
                model_A_cleint0 = PeftModel.from_pretrained(model_A, FL_args.cleint0_lora_modelA_path, device_map='cuda:0',trust_remote_code=True)
                model_A_cleint0 = model_A_cleint0.merge_and_unload()
                model_C_cleint0 = PeftModel.from_pretrained(model_C, FL_args.cleint0_lora_modelC_path, device_map='cuda:0',trust_remote_code=True)
                model_C_cleint0 = model_C_cleint0.merge_and_unload()
                model_A_cleint1 = PeftModel.from_pretrained(model_A, FL_args.cleint1_lora_modelA_path, device_map='cuda:0',trust_remote_code=True)
                model_A_cleint1 = model_A_cleint1.merge_and_unload()
                model_C_cleint1 = PeftModel.from_pretrained(model_C, FL_args.cleint1_lora_modelC_path, device_map='cuda:0',trust_remote_code=True)
                ...
                model_A_cleint0 = PeftModel.from_pretrained(model_A, FL_args.cleint0_lora_modelA_path, device_map='cuda:0',trust_remote_code=True)
                model_A_cleint0 = model_A_cleint0.merge_and_unload()
                model_A_cleint1 = PeftModel.from_pretrained(model_A_cleint0, FL_args.cleint1_lora_modelA_path, device_map='cuda:0',trust_remote_code=True)
                model_A = model_A_cleint1.merge_and_unload()
                model_C_cleint0 = PeftModel.from_pretrained(model_C, FL_args.cleint0_lora_modelC_path, device_map='cuda:0',trust_remote_code=True)
                model_C_cleint0 = model_C_cleint0.merge_and_unload()
                model_C_cleint1 = PeftModel.from_pretrained(model_C_cleint0, FL_args.cleint1_lora_modelC_path, device_map='cuda:0',trust_remote_code=True)
