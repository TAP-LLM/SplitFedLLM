...
class LlamaRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        input_dtype = hidden_states.dtype # torch.float32
        hidden_states = hidden_states.to(torch.float32)
        variance = hidden_states.pow(2).mean(-1, keepdim=True) # tensor([[[28.8765] torch.Size([1, 168, 1])
        hidden_states = hidden_states * torch.rsqrt(variance + self.variance_epsilon)
        # tensor([[[-0.0294, -0.0139, -0.0093,  ...,  0.0932,  0.0332, -0.0340]
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # tensor([[[-0.1580, -0.0748, -0.0502,  ...,  0.5009,  0.1782
        hidden_states = self.input_layernorm(hidden_states)
        # tensor([[[-8.7591e-04, -1.9539e-04, -2.8932e-05,
        # Self Attention
        ...
        # tensor([[[-1.7490e-01, -1.0596e-01, -3.6560e-02, 
        # Fully Connected
        residual = hidden_states
        hidden_states = self.post_attention_layernorm(hidden_states)
        hidden_states = self.mlp(hidden_states)
        hidden_states = residual + hidden_states
        #tensor([[[-1.2851e-01, -5.4482e-02, -6.0757e-03,  ...
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig, modelC_layers: int):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size
        #2025/3/15 change 分割点之前
        # self.layers = nn.ModuleList([LlamaDecoderLayer(config, 31)] )
        #2025/3/15 change 分割点之后
        self.layers = nn.ModuleList([LlamaDecoderLayer(config, layer_idx + 32-modelC_layers) for layer_idx in range(modelC_layers)])
    ...
    def forward(
        ...
        # past_seen_tokens = 1072 #CoQA一直都是这个值，训练用这个
        # past_seen_tokens = past_key_values.get_seq_length() #传pkv时设置
...
class LlamaForCausalLMC(LlamaPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config, modelC_layers):
        super().__init__(config)
        self.model = LlamaModel(config, modelC_layers)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # -1 表示让 PyTorch 自动计算出必要的维度大小，以保持原始数据的总量不变。PyTorch 会将 batch_size * sequence_length 作为新的第一维的大小，而第二维大小就是词汇表的大小 (vocab_size)。
            shift_labels = shift_labels.view(-1)
            # Enable model parallelism
            shift_labels = shift_labels.to(shift_logits.device)
            # tensor([ -100,     1,   518, 25580, 29962, 
        ...
        return CausalLMOutputWithPast(
            ...
            attentions=outputs.attention_mask,
        )
