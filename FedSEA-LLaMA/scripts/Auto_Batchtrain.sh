...
# ----------------- 超参数循环 -----------------
# cd到工作目录
cd /home/XXXX/SplitFederated-LLaMA/FedSEA-LLaMA
...
while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    Device='cuda:1'
    Device_client_0='cuda:1'
    Device_client_1='cuda:1'
    Torch_dtype="float16"
    ...
    python src/flclient_id_1.py \
        --do_train \
        --do_BatchTrain \
        --from_pickle \
        --data2pic \
        --device $Device_client_1 \
        --server_ip $ServerIP \
        --lora_r $Lora_r \
        --lora_alpha $Lora_alpha \
        --seed $Seed \
        ...
        --modelA_layers $ModelA_layers \
        --modelC_layers $ModelC_layers \
        --output_dir "$OUTPUT_DIR" > "$LOG_FILE_3" 2>&1 
