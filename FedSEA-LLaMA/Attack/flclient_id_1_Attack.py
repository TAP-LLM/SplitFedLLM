...
def FLparser(): 
    ...
    '''=====================================================model args====================================================='''                    
    # 
    parser.add_argument("--modelA_name_or_path", type=str, default="/home/XXXX/SplitFederated-LLaMA/Fed-Llama-module-autosplit/clientA",
                        help="The model checkpoint for weights initialization. Don't set if you want to train a model from scratch.")
    parser.add_argument("--modelC_name_or_path", type=str, default="/home/XXXX/SplitFederated-LLaMA/Fed-Llama-module-autosplit/clientC",
                        help="The model checkpoint for weights initialization. Don't set if you want to train a model from scratch.")
    ...
    parser.add_argument("--modelC_config_name", type=str, default=None,
                        help="Pretrained config name or path if not the same as model_name")
    parser.add_argument("--tokenizer_name", type=str, default="/home/XXXX/SplitFederated-LLaMA/Models/Llama2-7B-chat-service",
    ...
    #                     help="The pretrained model checkpoint for lora weights.")
    parser.add_argument("--lora_modelA_path", type=str, default="/home/XXXX/SplitFederated-LLaMA/Fed-Llama-module-autosplit/output/AttackExperiments/Version_0/client_0_model-A/checkpoint-8000")
    parser.add_argument("--lora_modelC_path", type=str, default="/home/XXXX/SplitFederated-LLaMA/Fed-Llama-module-autosplit/output/AttackExperiments/Version_0/client_0_model-C/checkpoint-8000")
    ...
    parser.add_argument("--output_dir", type=str, help = 'output folder path', default="/home/XXXX/SplitFederated-LLaMA/Fed-Llama-module-autosplit/output/BatchTrain")
    ...
    '''=====================================================datasets args====================================================='''  
    parser.add_argument("--data_fold", type=str, default="/home/XXXX/SplitFederated-LLaMA/Dataset/Record")
    ...
    parser.add_argument("--max_step", type=int, help='number of max training steps, should be same with serve side!', default=20000)
    ...
    parser.add_argument("--do_BatchTrain", action="store_true", help='Whether to inference using prompt from users.', default=False)
    ...
    parser.add_argument("--Attack_load_lora", action="store_true", help='Whether to attack to reconstruct dataset.', default=False)
    parser.add_argument("--Attack_lora_checkpoint", type=str, help="the number of the traing steps of every client", default="checkpoint-500")
...
class Fed_Client(NumPyClient):
    ...
    def train_fit_partA(self, parameters: NDArrays=None, current_step=0, need_test_data=0):
        ...
        # 保存feature看传输大小
        # stored_path = "/home/XXXX/SplitFederated-LLaMA/pickle-store/llama_fit_A.pickle"
    ...
    def train_fit_partC(self, parameters: NDArrays=None, config: Dict[str, Scalar]=None , child_conn=None):       
        ...
        #     plt.tight_layout()
        #     plt.savefig("/home/XXXX/SplitFederated-LLaMA/Fed-Llama-module/output/DP_debug/DP_Norm_per_step.png")
    ...
    def predict_fit_partA(self, parameters: NDArrays=None, current_step=0, need_test_data=0):
        ...
        # # 保存feature看传输大小
        # stored_path = "/home/XXXX/SplitFederated-LLaMA/Fed-Llama-module-autosplit/output/CoQA/debug/llama_fit_A.pickle"
        # with open(stored_path, 'wb') as f:
        #     pickle.dump(feature, f)
        #     logger.info(f"保存pickle数据完成")
        ...
        if pred_ids[0][-1] == 2 or len(self.total_pred) == self.FL_args.max_output_length:
            ...
            if self.FL_args.dataset_name == "MultiRC":
                ...
                if score is not None:
                    ...
                    # prediction_save_path = os.path.join(self.FL_args.lora_modelA_path,'FedSEA-llama_prediction.json')
                    if self.FL_args.add_DP_hidden:
...
def main(prompt: str =None , child_conn=None) -> None:
    ...
    # FL_args.data_fold = '/home/XXXX/SplitFederated-LLaMA/Dataset/Friends/ReadComprehension' #华佗数据集
    # FL_args.data_fold = "/home/XXXX/SplitFederated-LLaMA/Dataset/CoQA" # CoQA数据集
    ...
    if FL_args.do_train:
        ...
        for index in FL_args.target_modules:
            ...
            if index in global_modules:
                ...
                for i in clientA_layer_id:
                    t_m_c.append(f'model.layers.{i}.{index}')
            else:
        ...
        if FL_args.Attack and FL_args.Attack_load_lora:
            ...
            A_lora_path="/home/XXXX/SplitFederated-LLaMA/Fed-Llama-module-autosplit/output/Safety/version_1_Lr2e-5_Alayers5_Blayers5/model-A"
            C_lora_path="/home/XXXX/SplitFederated-LLaMA/Fed-Llama-module-autosplit/output/Safety/version_1_Lr2e-5_Alayers5_Blayers5/model-C"
            ...
            logger.info("检查点加载成功: model-A 和 model-C")
        ...
        if FL_args.custom_lora:
            ...
            Lora_A_state_dict = torch.load("/home/XXXX/SplitFederated-LLaMA/Fed-Llama-module-autosplit/lora_r16_alpha32_weights/lora_A/adapter_model.bin")
            ...
            Lora_C_state_dict = torch.load("/home/XXXX/SplitFederated-LLaMA/Fed-Llama-module-autosplit/lora_r16_alpha32_weights/lora_C/adapter_model.bin")
        ...
        if FL_args.Attack:
            ...
            if checkpoint_number ==500:
                A_lora_path="/home/XXXX/SplitFederated-LLaMA/Fed-Llama-module-autosplit/output/Safety/version_3_Lr2e-5_Alayers5_Blayers5/model-A"
    ...
    if not FL_args.from_pickle:
        ...
        if FL_args.data2pic:
            ...
            if FL_args.do_train:
                stored_path = f"/home/XXXX/SplitFederated-LLaMA/pickle-store/Distributed_{FL_args.dataset_name}_train_bs{FL_args.batch_size}_step{FL_args.max_step}.pickle"
            if FL_args.do_predict:
                if FL_args.iter_test:
                    stored_path = f"/home/XXXX/SplitFederated-LLaMA/pickle-store/Distributed_{FL_args.dataset_name}_IterTest.pickle"
                else:
                    stored_path = f"/home/XXXX/SplitFederated-LLaMA/pickle-store/Distributed_{FL_args.dataset_name}_FullTest.pickle"
            with open(stored_path, 'wb') as f:
        ...
        if FL_args.dataset_name == "CoQA":
            if FL_args.do_train:
                stored_path = f"/home/XXXX/SplitFederated-LLaMA/pickle-store/CoQA_Training_50000.pickle"
            if FL_args.do_predict:
                stored_path = f"/home/XXXX/SplitFederated-LLaMA/pickle-store/CoQA_TestDataset.pickle"
        if FL_args.dataset_name == "Xsum":
            if FL_args.do_train:
                if FL_args.Attack:
                    stored_path = "/home/XXXX/SplitFederated-LLaMA/pickle-store/Xsum_bs2_step10000_central_train_client1.pickle"
                else:
                    stored_path=f"/home/XXXX/SplitFederated-LLaMA/Models/central_model/finetune_code/pickle-store/Xsum_bs{FL_args.batch_size}_step{FL_args.max_step}_central_train.pickle"
            if FL_args.do_predict:
                if FL_args.iter_test:
                    stored_path="/home/XXXX/SplitFederated-LLaMA/Models/central_model/finetune_code/Xsum/Xsum_central_IterTest.pickle"
                else:
                    stored_path="/home/XXXX/SplitFederated-LLaMA/Models/central_model/finetune_code/Xsum/Xsum_central_FullTest.pickle"
        if FL_args.dataset_name == "MultiRC":
            if FL_args.do_train:
                if FL_args.Attack:
                    stored_path = f"/home/XXXX/SplitFederated-LLaMA/pickle-store/MultiRC_bs{FL_args.batch_size}_central_train_client1.pickle"
                else:
                    stored_path = f"/home/XXXX/SplitFederated-LLaMA/pickle-store/MultiRC_bs{FL_args.batch_size}_central_train.pickle"
            if FL_args.do_predict:
                if FL_args.iter_test:
                    stored_path = "/home/XXXX/SplitFederated-LLaMA/pickle-store/MultiRC_central_IterTest.pickle"
                else:
                    stored_path = "/home/XXXX/SplitFederated-LLaMA/pickle-store/MultiRC_central_FullTest.pickle"
        if FL_args.dataset_name == "Record":
            if FL_args.do_train:
                # 从中心化哪里来的数据
                stored_path = f"/home/XXXX/SplitFederated-LLaMA/pickle-store/MultiRC_bs2_central_train_client1.pickle"
                # stored_path = f"/home/XXXX/SplitFederated-LLaMA/pickle-store/Record_bs2_step20000_central_train_client1.pickle"
            if FL_args.do_predict:
                if FL_args.iter_test:
                    stored_path = "/home/XXXX/SplitFederated-LLaMA/Models/central_model/finetune_code/Record/Record_Central_IterTest.pickle"
                else:
                    stored_path = "/home/XXXX/SplitFederated-LLaMA/pickle-store/Record_Central_FullTest.pickle"
