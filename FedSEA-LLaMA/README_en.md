...
```
#### XSum Data Splitting
Refer to /Dataset/Xsum for details.

### Model Preparation
Note: Please cd to the working directory `cd /FedSEA-LLaMA`

1. Download the Llama 2 model and save it to `FedSEA-LLaMA/Centralized_Models/model_path/`
...
Modify the `cd /home/XXX/SplitFederated-LLaMA/FedSEA-LLaMA` in the bash script to the path where FedSEA-LLaMA is located.
Change `ServerIP` to your machine's IP address.Please note that all code has been anonymized, so running it directly will encounter path non-existence issues. Some manual path modifications are required before it can run normally.Due to the maximum capacity limitation of supplementary materials, only the Record dataset is provided for quick verification. The code clearly provides the complete acquisition methods for all datasets.
...
2. If not using `Auto_train.sh` to manually start the server and client, it is best to wait for the server to respond "Starting Flower server, config: ServerConfig(num_rounds=None, round_timeout=None)" before starting the client.
...
Notes:
1. Before starting the service, please modify all `'your_ip'` fields to your server and receiver's IP addresses.
2. If not using `train.sh` to manually start the server and client, it is best to wait for the server to respond "Starting Flower server, config: ServerConfig(num_rounds=None, round_timeout=None)" before starting the client.
