...
from get_dataloader import get_dataset_no_pad
from peft import (
...
def FLparser(): 
    ...
    '''=====================================================model args====================================================='''                    
    parser.add_argument("--modelA_name_or_path", type=str, default="./clientA",
                        help="The model checkpoint for weights initialization. Don't set if you want to train a model from scratch.")
    parser.add_argument("--modelC_name_or_path", type=str, default="./clientC",
                        help="The model checkpoint for weights initialization. Don't set if you want to train a model from scratch.")
    ...
    parser.add_argument("--modelC_config_name", type=str, default=None,
                        help="Pretrained config name or path if not the same as model_name")
    parser.add_argument("--tokenizer_name", type=str, default="Centralized_Models/model_path",
    ...
    parser.add_argument("--lora_modelC_path", type=str, default=None,
                        help="The pretrained model checkpoint for lora weights.")
    # parser.add_argument("--lora_modelA_path", type=str, default="./output/Record/version_3_r8_al32_Lr0.00003/model-A/checkpoint-33000")
    # parser.add_argument("--lora_modelC_path", type=str, default="./output/Record/version_3_r8_al32_Lr0.00003/model-C/checkpoint-33000")
    ...
    parser.add_argument("--output_dir", type=str, help = 'output folder path', default="output")
    ...
    '''=====================================================datasets args====================================================='''  
    parser.add_argument("--data_fold", type=str, default="./Dataset/Record")
    ...
    parser.add_argument("--from_pickle", action="store_true", help='Whether to run predictions on the test set.', default=False)
    ...
    '''=====================================================Fl Arguments====================================================='''  
    parser.add_argument("--server_ip", type=str, help='ip4_address:port.', default="10.143.12.73:7030")
    ...
    parser.add_argument("--client_id", type=int, default=0, choices=range(0, 10), required=False, help="Specifies the artificial data partition of CIFAR10 to be used. \
    ...
    '''=====================================================Attack_Arguments====================================================='''
    parser.add_argument("--Attack", action="store_true", help='Whether to attack to reconstruct dataset.', default=False)
...
class Fed_Client(NumPyClient):
    ...
    def train_fit_partA(self, parameters: NDArrays=None, current_step=0, need_test_data=0):
        ...
        # 保存feature看传输大小
        # stored_path = "./pickle-store/llama_fit_A.pickle"
    ...
    def train_fit_partC(self, parameters: NDArrays=None, config: Dict[str, Scalar]=None , child_conn=None):       
        ...
        #     plt.tight_layout()
        #     plt.savefig("./Fed-Llama-module/output/DP_debug/DP_Norm_per_step.png")
    ...
    def predict_fit_partA(self, parameters: NDArrays=None, current_step=0, need_test_data=0):
        ...
        # # 保存feature看传输大小
        # stored_path = "./output/CoQA/debug/llama_fit_A.pickle"
        # with open(stored_path, 'wb') as f:
        #     pickle.dump(feature, f)
        #     logger.info(f"保存pickle数据完成")
    ...
    def predict_fit_partC(self, parameters: NDArrays=None, config: Dict[str, Scalar]=None , child_conn=None):       
        ...
        # 解码预测结果
        tokenizer = AutoTokenizer.from_pretrained("Centralized_Models/model_path", trust_remote_code=True)
    ...
    def Batch_save_model(self, step):
        ...
        for i, model in enumerate(self.model):
            if i==0:
                check_fold = os.path.join(self.FL_args.output_dir, "client_0_model-A")
                model.save_pretrained(os.path.join(check_fold, 'checkpoint-{}'.format(str(step+1))))
            elif i==1: 
                check_fold = os.path.join(self.FL_args.output_dir, "client_0_model-C")
                model.save_pretrained(os.path.join(check_fold, 'checkpoint-{}'.format(str(step+1))))
        logger.info("Lora权重保存成功!") 
...
def main(prompt: str =None , child_conn=None) -> None:
    ...
    # FL_args.data_fold = './Dataset/Friends/ReadComprehension' #华佗数据集
    # FL_args.data_fold = "./Dataset/CoQA" # CoQA数据集
    ...
    if FL_args.do_train:
        ...
        if FL_args.custom_lora:
            logger.debug(f"Initial lora weights from bin")
            logger.debug(f"You need to check lora r and lora alpha")

            Lora_A_state_dict = torch.load("Lora_weights/lora_r16_alpha32_weights/lora_A/adapter_model.bin")
            ...
            Lora_C_state_dict = torch.load("Lora_weights/lora_r16_alpha32_weights/lora_C/adapter_model.bin")
        ...
        #4_11新增
        # 在 model_A.load_state_dict(adjusted_A_dict) 之后
        logger.info("Base model_A parameters(还没有加载LORA):")
        for name, param in model_A.named_parameters():
            logger.info(f"  {name}")
        # 在 model_C.load_state_dict(adjusted_C_dict) 之后
        logger.info("Base model_C parameters(还没有加载LORA):")
        for name, param in model_C.named_parameters():
            logger.info(f"  {name}")
        base_params = set([name for name, _ in model_A.named_parameters()])
        ...
        #4_11新增
        logger.info("LoRA adapter parameters for model_A:")
        for name, param in model_A.named_parameters():
            logger.info(f"  {name}")
        logger.info("LoRA adapter parameters for model_C:")
        for name, param in model_C.named_parameters():
            logger.info(f"  {name}")
        lora_params = set([name for name, _ in model_A.named_parameters()])
        missing_in_lora = base_params - lora_params
        extra_in_lora = lora_params - base_params
        logger.info(f"Params in base model_A but missing in LoRA: {missing_in_lora}")
        logger.info(f"Params in LoRA model_A but not in base: {extra_in_lora}")
    ...
    if not FL_args.from_pickle:
        ...
        if FL_args.data2pic:
            ...
            if FL_args.do_train:
                stored_path = f"./pickle-store/Distributed_{FL_args.dataset_name}_train_bs{FL_args.batch_size}_step{FL_args.max_step}.pickle"
            if FL_args.do_predict:
                if FL_args.iter_test:
                    stored_path = f"./pickle-store/Distributed_{FL_args.dataset_name}_IterTest.pickle"
                else:
                    stored_path = f"./pickle-store/Distributed_{FL_args.dataset_name}_FullTest.pickle"
            with open(stored_path, 'wb') as f:
        ...
        if FL_args.dataset_name == "CoQA":
            if FL_args.do_train:
                stored_path = f"./pickle-store/CoQA_Training_50000.pickle"
            if FL_args.do_predict:
                stored_path = f"./pickle-store/CoQA_TestDataset.pickle"
        if FL_args.dataset_name == "Xsum":
            if FL_args.do_train:
                if FL_args.Attack:
                    stored_path = "./pickle-store/Xsum_bs2_step10000_central_train_client0.pickle"
                else:
                    stored_path=f"./Models/central_model/finetune_code/pickle-store/Xsum_bs{FL_args.batch_size}_step{FL_args.max_step}_central_train.pickle"
            if FL_args.do_predict:
                if FL_args.iter_test:
                    stored_path="./Models/central_model/finetune_code/Xsum/Xsum_central_IterTest.pickle"
                else:
                    stored_path="./Models/central_model/finetune_code/Xsum/Xsum_central_FullTest.pickle"
        if FL_args.dataset_name == "MultiRC":
            if FL_args.do_train:
                if FL_args.Attack:
                    stored_path = f"./pickle-store/MultiRC_bs{FL_args.batch_size}_central_train_client0.pickle"
                else:
                    stored_path = f"./pickle-store/MultiRC_bs{FL_args.batch_size}_central_train.pickle"
            if FL_args.do_predict:
                if FL_args.iter_test:
                    stored_path = "./pickle-store/MultiRC_central_IterTest.pickle"
                else:
                    stored_path = "./pickle-store/MultiRC_central_FullTest.pickle"
        if FL_args.dataset_name == "Record":
            if FL_args.do_train:
                stored_path = f"./pickle-store/Distributed_{FL_args.dataset_name}_train_bs{FL_args.batch_size}_step{FL_args.max_step}.pickle"
            if FL_args.do_predict:
                if FL_args.iter_test:
                    stored_path = "./Models/central_model/finetune_code/Record/Record_Central_IterTest.pickle"
                else:
                    stored_path = "./pickle-store/Record_central_FullTest.pickle"
...
if __name__ == "__main__":
    ...
    if (args.add_DP_gradient or args.add_DP_hidden ) and not args.Attack:
        log_path = os.path.join(args.output_dir, f'DP_no_attack_client_0_noise{args.grad_noise}_Alayers{args.modelA_layers}_Clayers{args.modelC_layers}.log')
    elif (args.add_DP_gradient or args.add_DP_hidden ) and args.Attack:
        log_path = os.path.join(args.output_dir, f'Attack_with_DP_client_0_noise{args.grad_noise}_Alayers{args.modelA_layers}_Clayers{args.modelC_layers}.log')
    elif not (args.add_DP_gradient or args.add_DP_hidden ) and args.Attack:
        log_path = os.path.join(args.output_dir, f'Attack_no_DP_client_0_Alayers{args.modelA_layers}_Clayers{args.modelC_layers}.log')
    else:
        log_path = os.path.join(args.output_dir, f'client_0_Alayers{args.modelA_layers}_Clayers{args.modelC_layers}.log')
