...
def FLparser(): 
    ...
    parser.add_argument("--max_step", type=int, help='number of max training steps, should be same with serve side!', default=10000)
...
class Fed_Client(NumPyClient):
    ...
    def predict_fit_partC(self, parameters: NDArrays=None, config: Dict[str, Scalar]=None , child_conn=None):       
        ...
        # 解码预测结果
        tokenizer = AutoTokenizer.from_pretrained("./Models/Llama2-7B-chat-service", trust_remote_code=True)
...
def main(prompt: str =None , child_conn=None) -> None:
    ...
    if FL_args.do_train:
        ...
        if FL_args.Attack and FL_args.Attack_load_lora:
            logger.warning("加载lora检查点使得客户端1与客户端0不同!")
            ...
            if checkpoint_number ==500:
                A_lora_path="./output/Safety/version_3_Lr2e-5_Alayers5_Blayers5/model-A"
                logger.info(f"A_lora_path:{A_lora_path}")
            elif checkpoint_number ==4000:
                A_lora_path="./output/Safety/version_1_Lr2e-5_Alayers5_Blayers5/model-A"
                logger.info(f"A_lora_path:{A_lora_path}")
            else:
                logger.warning("请检查A_lora_path路径!")

            # C_lora_path="./output/Safety/version_1_Lr2e-5_Alayers5_Blayers5/model-C"
    ...
    if not FL_args.from_pickle:
        ...
        if FL_args.dataset_name == "Xsum":
            if FL_args.do_train:
                if FL_args.Attack:
                    stored_path = "./pickle-store/Xsum_bs2_step10000_central_train_client1.pickle"
                else:
        ...
        if FL_args.dataset_name == "MultiRC":
            if FL_args.do_train:
                if FL_args.Attack:
                    stored_path = f"./pickle-store/MultiRC_bs{FL_args.batch_size}_central_train_client1.pickle"
                else:
        ...
        if FL_args.dataset_name == "Record":
            if FL_args.do_train:
                # 从中心化哪里来的数据
                # stored_path = f"./Models/central_model/finetune_code/pickle-store/Record_bs{FL_args.batch_size}_step{FL_args.max_step}_central_train.pickle"
                stored_path = f"./pickle-store/Distributed_{FL_args.dataset_name}_train_bs{FL_args.batch_size}_step{FL_args.max_step}.pickle"
            if FL_args.do_predict:
                if FL_args.iter_test:
