...
def FLparser(): # use glm arguments with argument.py
    ...
    # parser.add_argument("--lora_modelB_path", type=str, default="srcsrc/output/Record/version_3_r8_al32_Lr0.00003/model-B/checkpoint-33000", help="The pretrained model checkpoint for lora weights.")
    ...
    parser.add_argument("--Attack_model_path", type=str, default="/home/XXXX/SplitFederated-LLaMA/Models/central_model/model_path")
    ...
    def train(self, client_instructions, num_rounds, timeout, start_time, history):
        ...
        for client_proxy, ins in client_instructions:          
            for current_round in range(num_rounds):
                ...
                # #查看精度损失
                # stored_path = "/home/XXXX/SplitFederated-LLaMA/pickle-store/featureA_ndarrays_server.pickle"
        ...
        for current_round in range(num_rounds):
            ...
            if self.model_args.Attack and (self.attack_decoder_model is not None):
                ...
                if self.global_step % 2000 == 0:
                    #保存模型检查点
                    attack_ckpt=f"srcsrc/output/Attack_decoder_checkpoint/version_{self.model_args.Attack_version}_step-{self.global_step}.pth"
            ...
            while True:
                ...
                if self.model_args.start_inference == "OK":
                    ...
                    for inference_step in range(self.model_args.max_output_length):
                        ...
                        feature_array = [val.cpu().numpy() for _, val in featureB_od.items()]
                        # 保存feature看传输大小
                        # stored_path = "srcsrc/output/CoQA/debug/llama_fit_B.pickle"
                        # with open(stored_path, 'wb') as f:
...
def main(child_conn_server=None):
    ...
    if type(FL_args.target_modules)==str:
        ...
        if len(FL_args.target_modules)==8:
            ...
            logger.debug("多客户端训练时客户端1可变词表!")
        elif len(FL_args.target_modules)==4:
            FL_args.Client1_VariedEmbeding=False
            logger.debug("多客户端训练时客户端1不可变词表!")
        else:
            logger.debug("请检查target_modules!")
    ...
    if FL_args.Attack and FL_args.Attack_wandb:
        ...
        wandb.init(
            ...
            name=f"version_{FL_args.Attack_version}_noise_{FL_args.grad_noise}_Alayers_{FL_args.modelA_layers}_Attacklora_{FL_args.Attack_lora_checkpoint}_VariedEmbeding_{FL_args.Client1_VariedEmbeding}",
    ...
    if FL_args.Attack:
        ...
        class LlamaAttackDecoder(nn.Module):
            ...
            def load_checkpoint(self, checkpoint_path: str, map_location='cpu'):
                ...
                :param map_location: 加载到的设备（'cpu', 'cuda:1'等），默认加载到CPU
                """
    ...
    if FL_args.do_train or FL_args.do_BatchParallel_train:   
        ...
        if FL_args.custom_lora:
            ...
            Lora_B_state_dict = torch.load("Lora_weights/lora_r16_alpha32_weights/lora_B/adapter_model.bin")
