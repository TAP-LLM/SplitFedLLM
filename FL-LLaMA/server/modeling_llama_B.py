...
#     app.run(host='10.143.12.74', port=5001)
import pdb
...
class LlamaSdpaAttention(LlamaAttention):
    ...
    def forward(
        ...
        cos, sin = self.rotary_emb(value_states, position_ids)
        # value states torch.Size([2, 32, 920, 128])  position_ids torch.Size([1, 920])
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin)
        ...
        if past_key_value is not None:
            ...
            key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)
        # key_cache:
        #[tensor([[[[-0.2559, ...='cuda:0'), tensor([[[[-5.2967e-...ackward0>)]
        # seen_tokens:
        # tensor(168)
        # value_cache:
        # [tensor([[[[ 6.3417e-...='cuda:0'), tensor([[[[ 1.3514e-...ackward0>)]
        ...
        attn_output = torch.nn.functional.scaled_dot_product_attention(
            query_states, #torch.Size([4, 32, 920, 128])
            key_states, # torch.Size([4, 32, 920, 128])
            value_states, # torch.Size([4, 32, 920, 128])
            attn_mask=causal_mask, # torch.Size([4, 1, 920, 920])
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig, modelA_layers:int, modelC_layers:int):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size

        #没有embedding层
        # self.layers = nn.ModuleList([LlamaDecoderLayer(config, layer_idx) for layer_idx in range(1,31)])
        #2025/3/15 change 分割点之后
        self.layers = nn.ModuleList([LlamaDecoderLayer(config, layer_idx) for layer_idx in range(modelA_layers, 32-modelC_layers)])
    ...
    def forward(
        ...
        # 模型B拿不到原始输入，所有不需要对输入进行嵌入

        
        # past_seen_tokens = past_key_values.get_seq_length() #10.31以前都用的是这一版
        # past_seen_tokens = 1072 #CoQA 数据目前这一版为1072训练中保持不变
        # 这里在train的时候，seq_len保持不变，为 max_source_length+max_target_length
        past_seen_tokens = hidden_states.shape[1]# 不传pkv时的设置，测试用这个
        ...
        #     )
        if cache_position is None:
            # inputs_embeds 表示当前输入的 token 的嵌入，它的形状通常为 (batch_size, seq_len, embedding_dim)
            # 使用输入的 token 长度来生成 cache_position
            #seq_length = hidden_states.shape[1]  # 假设 hidden_states 是 (batch_size, seq_len, hidden_dim) ([1, 168, 4096])
            # cache_position = torch.arange(
            #     past_seen_tokens, past_seen_tokens + seq_length, device=hidden_states.device
            # )
            cache_position = torch.arange(
                0, past_seen_tokens, device=hidden_states.device
            )
        ...
        for i , decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                # if i ==0:
                #     stored_path = f"/home/zhangzishuai/SplitFederated-LLaMA/pickle-store/model_B_input_hiddenstates.pickle"
                #     with open(stored_path, 'wb') as f:
                #         pickle.dump(hidden_states, f)
                #         logger.info(f"保存model_B_input_hiddenstates数据完成")

                layer_outputs = decoder_layer(
                        hidden_states, # 第一层[[[ 0.0009,  0.0014,  0.0391,  ...,  0.0357, -0.0052,  0.0301],
            ...
            # print(f"i:{i+1},hidden_states:{hidden_states}")
            if use_cache:
    ...
    def _update_causal_mask(self, attention_mask, hidden_states): # no use
        #region
        ...
        # 如果启用了 flash_attention_2 并且 attention_mask 存在填充值 0.0，直接返回 attention_mask
        if self.config._attn_implementation == "flash_attention_2":
            if attention_mask is not None and 0.0 in attention_mask:
                return attention_mask
            return None
        ...
        # 如果序列长度超出了缓存的最大位置编码，更新 causal_mask
        if seq_length > self.causal_mask.shape[-1]:
            causal_mask = torch.full((2 * self.causal_mask.shape[-1], 2 * self.causal_mask.shape[-1]), fill_value=1)
            self.register_buffer("causal_mask", torch.triu(causal_mask, diagonal=1), persistent=False)

        # 确保 causal_mask 是最新的 dtype 以避免溢出
        ...
        # 如果 attention_mask 存在，并且是 2 维的，进行处理
        if attention_mask is not None and attention_mask.dim() == 2:
            mask_length = attention_mask.shape[-1]
            # 生成填充掩码
            padding_mask = causal_mask[..., :mask_length].eq(0.0) * attention_mask[:, None, None, :].eq(0.0)
        ...
        # 处理 sdpa (scaled dot-product attention) 的情况
        if self.config._attn_implementation == "sdpa":
            is_tracing = torch.jit.is_tracing() or isinstance(hidden_states, torch.fx.Proxy)
            if not is_tracing and attention_mask is not None and torch.any(attention_mask != 1):
                # 修改因果掩码，确保只保留有效的注意力部分
                causal_mask = causal_mask.mul(~torch.all(causal_mask == causal_mask.min(), dim=-1)[..., None]).to(dtype)

        return causal_mask


class LlamaForCausalLMB(LlamaPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config, modelA_layers, modelC_layers):
        super().__init__(config)
        self.model = LlamaModel(config, modelA_layers, modelC_layers)
        self.vocab_size = config.vocab_size
