...
from client_partA_config import ClientChatGLMConfig
...
class ChatGLMModelClientSide(ChatGLMPreTrainedModel):
    """
    ...
    def __init__(self, config: ClientChatGLMConfig, empty_init=True):
    ...
    def forward(
        ...
        if self.gradient_checkpointing and self.training: # 无用
            if use_cache:
                ...
                use_cache = False # 所以use_cache一直是True
        ...
        # )
        pkv = past_key_values[self.client_num_layers:]
...
class ChatGLMForConditionalGenerationClientSide(ChatGLMPreTrainedModel): # 先站在巨人的肩膀上改吧 原函数是ChatGLMForConditionalGeneration
    def __init__(self, config: ClientChatGLMConfig, empty_init=True): # 看看是否需要增加参数 config得改吧 client_config.py
        super().__init__(config)
        if empty_init:
            init_method = skip_init
        else:
        ...
        self.transformer = ChatGLMModelClientSide(config, empty_init=empty_init)  # changed!!
    ...
    def forward( 
        ...
        # lm_logits = self.lm_head(hidden_states).permute(1, 0, 2).contiguous() # 这是模型的最后一层
