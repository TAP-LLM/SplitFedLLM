...
def FLparser(): # use glm arguments with argument.py
    ...
    '''=====================================================model args====================================================='''                    
    # 
    parser.add_argument("--modelB_name_or_path", type=str, default="/home/zhangzishuai/SplitFederated-LLaMA/Fed-Llama-module-autosplit/server",
    ...
    # parser.add_argument("--lora_modelB_path", type=str, default="/home/zhangzishuai/SplitFederated-LLaMA/Fed-Llama-module-autosplit/output/Record/version_3_r8_al32_Lr0.00003/model-B/checkpoint-33000", help="The pretrained model checkpoint for lora weights.")
    parser.add_argument("--lora_modelB_path", type=str, default=None, help="The pretrained model checkpoint for lora weights.")
    ...
    parser.add_argument("--output_dir", type=str, help = 'output folder path', default="/home/zhangzishuai/SplitFederated-LLaMA/Fed-Llama-module/Ablation_study")
    ...
    parser.add_argument("--max_step", type=int, help='number of max training steps, should be same with server side!', default=15000)
    ...
    parser.add_argument("--do_BatchParallel_train", action="store_true", help='Whether to run BatchParallel training.', default=True)
    ...
    parser.add_argument("--client_count", type=int, help='number of clients', default=2)
    ...
    parser.add_argument("--Attack_Lora", action="store_true", help='Whether to use lora to update attack decoder.', default=False)
    ...
    parser.add_argument("--Attack_random_initial", type=lambda x: x.lower() == 'true', help='Whether to initial decoder from random or Llama', default=True)
    parser.add_argument("--Attack_wandb", action="store_true", help='Whether to attack to reconstruct dataset.', default=True)
    parser.add_argument("--Attack_predict_next", type=lambda x: x.lower() == 'true', help='Whether to attack to reconstruct dataset.', default=True)
    parser.add_argument("--Attack_no_mask", action="store_true", help='Whether to attack to reconstruct dataset.', default=False)
    parser.add_argument("--Attack_truncation", action="store_true", help='Whether to attack to reconstruct dataset.', default=True)
    ...
    parser.add_argument("--Attack_version", type=int, help="the number of the traing steps of every client", default=99)
...
class FL_Server(Server):
    ...
    def __init__(self, optimizer,schedule,ServerModel, args, logger, client_manager: ClientManager, strategy: Strategy = None, child_conn_server=None,attack_decoder_model=None,attack_optimizer=None,attack_tokenizer=None) -> None:
        ...
        if self.model_args.do_train or self.model_args.do_BatchParallel_train:
            ...
            logger.info(f"下面是服务器端 模型B的可训练参数")
            logger.info("\n")  
        ...
        for current_round in range(num_rounds):
            ...
            if self.model_args.Attack and (self.attack_decoder_model is not None):
                ...
                if self.global_step % 2000 == 0:
                    #保存模型检查点
                    attack_ckpt=f"/home/zhangzishuai/SplitFederated-LLaMA/Fed-Llama-module-autosplit/output/Attack_decoder_checkpoint/version_{self.model_args.Attack_version}_step-{self.global_step}.pth"
                    self.attack_decoder_model.save_checkpoint(attack_ckpt)
            ...
            while True:
                ...
                if self.model_args.start_inference == "OK":
                    ...
                    for inference_step in range(self.model_args.max_output_length):
                        ...
                        feature_array = [val.cpu().numpy() for _, val in featureB_od.items()]
                        # 保存feature看传输大小
                        # stored_path = "/home/zhangzishuai/SplitFederated-LLaMA/Fed-Llama-module-autosplit/output/CoQA/debug/llama_fit_B.pickle"
                        # with open(stored_path, 'wb') as f:
...
def main(child_conn_server=None):
    ...
    if type(FL_args.target_modules)==str:
        ...
        if len(FL_args.target_modules)==7:
    ...
    if FL_args.Attack and FL_args.Attack_wandb:
        ...
        wandb.init(
            ...
            project=f"Formal_Distributed_Attack_Experiment",
            name=f"version_{FL_args.Attack_version}_nextpred_{FL_args.Attack_predict_next}_noise_{FL_args.grad_noise}_Alayers_{FL_args.modelA_layers}_Attacklora_{FL_args.Attack_lora_checkpoint}_VariedEmbeding_{FL_args.Client1_VariedEmbeding}",
    ...
    if FL_args.Attack:
        ...
        class LlamaAttackDecoder(nn.Module):
            ...
            def save_checkpoint(self, save_path: str):
                ...
                :param save_path: 用于保存模型参数的文件路径（例如 'decoder_checkpoint.pth'）。
                """
                # 如果目录不存在，自动创建
            ...
            def load_checkpoint(self, checkpoint_path: str, map_location='cpu'):
                """
                从指定文件加载本模型的参数，仅恢复模型权重。
                
                :param checkpoint_path: 存有模型参数的 .pth 文件路径
                :param map_location: 加载到的设备（'cpu', 'cuda:0'等），默认加载到CPU
                """
    ...
    if FL_args.do_train or FL_args.do_BatchParallel_train:   
        ...
        for index in FL_args.target_modules:
            ...
            if index in global_modules:
                ...
                for i in server_layer_id:
                    t_m_s.append(f'model.layers.{i}.mlp.{index}')
            else:
        ...
        if FL_args.custom_lora:
            ...
            Lora_B_state_dict = torch.load("/home/zhangzishuai/SplitFederated-LLaMA/Fed-Llama-module-autosplit/lora_r8_alpha16_weights/lora_B/adapter_model.bin")
