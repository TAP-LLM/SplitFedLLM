...
# 定义版本路径
Version_dir="/home/zhangzishuai/SplitFederated-LLaMA/Fed-Llama-module/output/Xsum/version_7_r16_alpha32_lr0.00002"

# 遍历所有检查点
for CheckpointPath in 48000 41000; do
    # 提取检查点编号
    CheckpointStep=$CheckpointPath

    echo "进行检查点 ${CheckpointStep} 的测试"

    # # 删除目标文件（使用绝对路径，添加错误抑制）
    # rm -f "${CheckpointPath}/prediction_kvcache.json" 2>/dev/null
    # echo "已清理检查点 $CheckpointStep 的旧文件"

    # 获取当前时间戳
    TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
    Data_fold="/home/zhangzishuai/SplitFederated-LLaMA/Dataset/Xsum/xsum"
    ...
    # 定义日志文件路径
    LOG_FILE_1="$Output_dir/predict_output_server.log"
    # 检查日志文件是否存在，如果存在则删除
    if [ -f "$LOG_FILE_1" ]; then
        rm "$LOG_FILE_1"
    fi

    LOG_FILE_2="$Output_dir/predict_output_client.log"
    # 检查日志文件是否存在，如果存在则删除
